...
同じ形式で並べる。別のタブで応答が受け入れられると、リクエスト画面は次のリクエストのために新しい
nonce と state を用意する。結果画面は同じ nonce を二度受け入れない。結果画面が応答を判定するのは
読み込んだときの一度だけで、言語や照合の切り替えは描き直すだけ。規則の全体と、それをブラウザの中で
守る理由は [決定 0015](../docs/decisions/0015-rp-accepts-one-response-per-request.md)(英語)にある。

### 照合の違いを試す

届いたトークンは変えられないが、RP が期待する値は変えられる。検証結果の画面では、同じトークンを
別の `nonce`・`state`・`aud` で照合し直せるので、ひとつの検査が失敗し、残りは通ったまま表示される
ところを確かめられる。トークン自体には手を付けない。差し替えた期待値での照合は、結果がどうであれ
認証にはならない。
...
- `test/pending.test.mjs` — 一つのリクエストに受け入れる応答は一つだけ。二つのタブが同時に取りに
  行っても変わらない。控えの確認と削除は、同じオリジンのすべてのタブが共有する Web Lock の中で行い、
  Web Locks が無いブラウザではどの応答も受け入れない。一度受け入れた nonce は、そのトークンが
  有効な間は、ほかにいくつ受け入れても、控えが書き戻されても、二度と受け入れない
- `test/callback.test.mjs` — 結果画面そのものを、DOM・ストレージ・ロックの代役で動かす。判定は
  一度だけ。検証やロックを待つ間に言語や照合を切り替えても、受け入れを失わず、差し替えた期待値で
  成功と示すことも受け入れることもない。フラグメントが変われば、古い応答では受け入れずに読み込み直す
- `test/app.test.mjs` — リクエスト画面が控えを書くのは値が変わったときだけ。言語を切り替えても
  使い終わったリクエストは戻らず、別のタブが控えを消したという遅れた通知で新しい控えを上書きしない
