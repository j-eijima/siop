...
未実装。両アプリは `client_id` ごとに別の鍵を持って自動で使うため、pairwise な `sub` は
すでに成立しているが、ユーザーに選択肢は出ていない。この仕様を満たすとは、その動作を
置き換えることではなく拡張すること。ある RP に紐づいた識別子は、ユーザーが変えない限り
紐づいたままにする。意図する形は [UI モック](../../../rp/public/mock.html)が
一時的なメモリ上の鍵で示している。
