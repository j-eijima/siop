...
両側とも実装済み。iOS アプリは受け取ったリクエストの各パラメータを届いた値のまま示し、応答の
プレビューの各値に出どころを付けている(Android はまだ)。`rp/` のテスト RP は、送信前に
リクエストを示し、応答と照合する値に印を付け、検査ごとに期待値と並べて示す。
[HTML UI モック](../../../rp/public/mock.html)は両方を一時的なメモリ上の鍵で示すデモであって、
相互運用テストではない。
